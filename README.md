# Fritter (~2013-2014)
    ...
    CMS lleuger fet en PHP per crear fàcilment we sites dinàmics amb:

    - Backend en PHP 5.2+, MySQL opcional
    - Frontend amb jQuery 1.9 i Gumby CSS Framework
    - Panell d'administració per gestionar usuaris, mòduls dinàmics i configuració
    - Sistema de mòduls definits via JSON (no cal tocar codi per crear nous tipus de contingut)

    Arquitectura MVC rudimentari: application/run.php fa de router, application/libs/main.php és el core, i les vistes són fitxers .phtml.
